...
"""
Этот модуль предоставляет функции для генерации случайных данных с использованием библиотеки Faker и различных предопределенных списков. Эти функции генерируют случайные записи для различных сущностей, таких как покупки, сотрудники, покупатели, игры, игровые платформы, разработчики и жанры.

Функции:
- `RandomPurchase(GameIdLimit, EmployeeIdLimit, BuyerIdLimit)`: Генерирует случайную запись о покупке с указанными ограничениями по ID игр, сотрудников и покупателей.
- `RandomEmployee()`: Генерирует случайную запись о сотруднике.
- `RandomBuyer(PlatformIdLimit, GenreIdLimit)`: Генерирует случайную запись о покупателе с указанными ограничениями по ID платформ и жанров.
- `RandomGame(PlatformIdLimit, GenreIdLimit, DeveloperIdLimit)`: Генерирует случайную запись о игре с указанными ограничениями по ID платформ, жанров и разработчиков.
- `RandomPlatform()`: Генерирует случайную запись о игровой платформе.
- `RandomDeveloper()`: Генерирует случайную запись о разработчике.
- `RandomGenre()`: Генерирует случайную запись о жанре.

Переменные:
- `fake`: Объект Faker для генерации случайных данных.
- `names`: Список имен для генерации случайных имен.
- `gameNames1`, `gameNames2`: Списки частей названий для генерации случайных названий игр.
- `prices`: Список возможных цен для генерации случайных цен игр.
- `ageRatings`: Список возрастных рейтингов для генерации случайных возрастных рейтингов игр.
- `languages`: Список языков для генерации случайных языков игр.
- `platform1`, `platform2`: Списки частей названий для генерации случайных названий платформ.
- `dev1`, `dev2`: Списки частей названий для генерации случайных названий разработчиков.
- `gameGenres`: Список жанров для генерации случайных жанров игр.
"""
